...
def profile_update(request):
    if request.is_ajax() and request.method == 'POST':
        try:
            ...
            print(str(e))
            return JsonResponse({"successFlag": False, "error": "Exception", "message": str(e)})
    else:
                    ...
                    try:
                        ...
                        if user:
                            ...
                            if user_profile:
                                ...
                                link = f"{Base_url}account/account_activate?email={email}&act_link={activation_code}"

                                # message setting for self mail
                                msg = MIMEMultipart()
                                msg['To'] = email
                                msg['Subject'] = f"Riddle Champs - Activate your Account " + emoji.emojize(":sign_of_the_horns:")

                                body = f"Hello<p>Thankyou for making account with us. <p>Please <a href='{link}'>click here</a> to activate your account.<p><br />--<br />{settings.EMAIL_SIGNATURE}"

                                msg.attach(MIMEText(body, 'html'))

                                cvEmailer.sendMultiMediaMail(email, msg)
                ...
                try:
                    ...
                    if len(user_obj) > 0:
                        try:
                            ...
                            if pr_obj:
                                link = f"{Base_url}account/reset_password?email={email}&token={token}"

                                # message setting for self mail
                                msg = MIMEMultipart()
                                msg['To'] = email
                                msg['Subject'] = "Riddle Champ - Rest your Password " + emoji.emojize(":sign_of_the_horns:")

                                body = f"Hello, <p>Please click on <a href='{link}'>click here</a> to reset your password. Please note the link will expire in 30 mins.<p><br />--<br />{settings.EMAIL_SIGNATURE}"
...
def resent_activation_mail(request):
    if request.is_ajax():
        ...
        if request.method == 'GET' and 'email' in request.GET and 'act_link' in request.GET:
            ...
            link = f"{Base_url}account/account_activate?email={email}&act_link={act_code}"
            # message setting for self mail
            msg = MIMEMultipart()
            msg['To'] = email
            msg['Subject'] = "Riddle Champ - Activate your Account " + emoji.emojize(":sign_of_the_horns:")

            body = f"Hello, Thankyou for making account with us.<p>Please <a href='{link}'>click here</a> to activate your account.<p><br />--<br />{settings.EMAIL_SIGNATURE}"
