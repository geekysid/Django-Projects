...
from .models import RiddleCategory, RiddleType, RiddleLevel, Riddle, Den, DenRiddle, Response, Hunter_Den_Mapping
...
class RiddleAdmin(admin.ModelAdmin):
    list_display = ('riddle_id', 'category', 'riddle_level', 'type_riddle', 'media', 'question', 'answer_1', 'answer_2', 'answer_3', 'answer_4', 'answer_5', 'answer_6', 'answer_7', 'answer_8', 'answer_9')
...
class DenAdmin(admin.ModelAdmin):
    list_display = ('den_id', 'name', 'admin', 'desc', 'avatar', 'score', 'riddle_start_time', 'riddles_per_day', 'time_bw_riddle', 'is_active', 'started_at', 'ended_at', 'invitation_code')
...
class Hunter_Den_MappingAdmin(admin.ModelAdmin):
    list_display = ('map_id', 'den', 'hunter')
...
class DenRiddleAdmin(admin.ModelAdmin):
    list_display = ('den_riddle_id', 'den', 'riddle', 'added_at', 'started_at', 'ending_at', 'is_pending', 'is_active', 'has_expired')
    list_filter = ('den', 'riddle', 'is_pending', 'is_active', 'has_expired')
    ordering = ('den', 'riddle', 'started_at', 'ending_at',)
    search_fields = ('den', 'riddle', 'is_pending', 'is_active', 'has_expired')
