...
import zlib, hashlib, time, os, smtplib, emoji
...
def index(request):
    if request.user.is_authenticated:
        ...
        userDen_list = []
        user_den_map_obj = Hunter_Den_Mapping.objects.filter(hunter=userProfile_obj)
        ...
        if user_den_map_obj.count() > 0:
            ...
            if not df_userResponse.empty:
                ...
                riddlesSovled = df_userResponse[(df_userResponse['is_correct'] == True)]['is_correct'].count()
                riddlesUnsovled = len(den_riddle_obj) - riddlesSovled
            
                userResponse = {
                    "totalScore" : totalScore,
                    "riddlesSovled" : riddlesSovled,
                    "riddlesUnsovled": riddlesUnsovled
                }
            else:
                userResponse = {
                    "totalScore" : 0,
            ...
            for den in denDetails_obj:
                ...
                if not riddle_list.empty and not df_userResponse.empty:
                    ...
                    print(f"{den} - Riddle: {len(riddle_list)} - Solved: {solved_riddle_count}")
                ...
                if len(user_den_map) == 1:
                    ...
                    for riddle_den in riddle_den_s:
                        ...
                        riddle_den_dict = {
                            ...
                            'right_ans_score': riddle_den.riddle.riddle_level.positive_score_percent*__BASE_SCORE,
                            'score_percent': (user_score_riddle*100)/(riddle_den.riddle.riddle_level.positive_score_percent*__BASE_SCORE),
                            'wrong_ans_score': riddle_den.riddle.riddle_level.negetive_score_percent*__BASE_SCORE
                        }
                    ...
                    Top_skRatio_score = 0
                    Top_skRatio_name = userProfile_obj.name
                    ...
                    for hunter_den in hunters_den_obj:
                        ...
                        if df_response.empty:
                            ...
                            if hunter_den.hunter.user.id in df_response['hunter_id'].values:
                                ...
                                hunters_wrong_resp = df_response.loc[condition, 'is_correct'].count()
                        ...
                        if hunters_total_resp > 0:
                            ...
                            if skRatio_score > Top_skRatio_score:
                                ...
                                Top_skRatio_name = hunter_den.hunter.name
                                Top_skRatio_avatar = hunter_den.hunter.avatar

                            elif (hunters_correct_resp == 0) and (skRatio_score == 0):
                                if Top_skRatio_name == None:
                                    Top_skRatio_name = hunter_den.hunter.name
                                if Top_skRatio_name == None:
                                    Top_skRatio_avatar = hunter_den.hunter.avatar
                    ...
                    den_details = { 
                        ...
                        'total_riddles': total_riddles,  # number of total riddels asked
                        "name" : den.name,                      # DEN
                        ...
                        "Top_skRatio_name": Top_skRatio_name,
...
def denRiddle(request):
    ...
    if request.user.is_authenticated:
            ...
            if len(den_riddle_obj) == 1:
                ...
                if len(hunter_den_map_obj) == 1:
                    ...
                    params = {
                            ...
                            'right_ans_score': den_riddle.riddle.riddle_level.positive_score_percent*__BASE_SCORE,
                            'wrong_ans_score': den_riddle.riddle.riddle_level.negetive_score_percent*__BASE_SCORE,
                            'IS_MOBILE': IS_MOBILE
                        }
        ...
        #  when user sumits an answer
            ...
            den_riddel = DenRiddle.objects.filter(uin_code=uin_code)
            
            if len(den_riddel) == 1:

                den_riddel = den_riddel[0]
                answer = request.POST.get('answer', None).lower().strip().replace(" ", "")

                # answer = 'Pencil lead'.lower()
                if (
                    den_riddel.riddle.answer_1.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_2.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_3.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_4.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_5.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_6.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_7.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_8.lower().strip().replace(" ", "") == answer or 
                    den_riddel.riddle.answer_9.lower().strip().replace(" ", "") == answer
                ):
                    score = round(den_riddel.riddle.riddle_level.positive_score_percent*__BASE_SCORE, 2)
                    is_correct = True
                    result = "?s=301"
                else:
                    score = round(den_riddel.riddle.riddle_level.negetive_score_percent*__BASE_SCORE, 2)
                    is_correct = False
                    result = "?e=901"
                    
                current_time_epoch = int(datetime.now().strftime('%s'))
                activate_time_epoch = int(den_riddel.started_at.strftime('%s'))
                    ...
                    if len(hunter_den_map_obj) == 1:
                        ...
                        if hunter_den_map.member_status == True:
                            ...
                            if hunter_den_map.den.admin == request.user:
                                ...
                                subject = "Hunterz Den - Approved to join Den {hunter_den_map.den}  {sender_obj.name} {emoji.emojize(':sign_of_the_horns:')}"
                                ...
                                mail_sent = mailer(subject, '', to_, body)
            ...
            if den_code == '':
                ...
                if len(den_obj) == 1:
                    ...
                    if len(hunter_den_map_obj) > 0:
                        ...
                        confirmation_link = f"http://127.0.0.1:8000/riddlechamp/den/join/?cl={invitation_code}"
                        subject = f"Hunterz Den - {hunter} Requests to Join Your Den ({den}) {emoji.emojize(':sign_of_the_horns:')}"
                        body = f"Hello,<p>\
                                {joinee}({joinee_email}) have requested to join your den ({den}).</p>\
                                <p><a href='{confirmation_link}'>Click here</a> to allow him to be part of your Den</p>"

                        mail_sent = mailer(subject, joinee_email, admin_email, body)
    ...
    if request.user.is_authenticated:
        if request.is_ajax():
            if request.method == 'POST':
                ...
                subject = "Hunterz Den - Invitation to join DEN by " + sender_obj.name + emoji.emojize(":sign_of_the_horns:")
                ...
                for email in email_list:
                    ...
                    if len(guest_obj) == 0:
                        ...
                        return JsonResponse({'success': False, 'error': 'NEW USER', 'message': 'The email address is not a Hunter yet. We have sent him a link to register. Once he accepts, you can sent him a fresh invite to this den.'})
                    else:
                        ...
                        if len(hunter_den_map_obj) == 0:
                            ...
                            invitation_link = "http://127.0.0.1:8000/riddlechamp/den/invite/guest?email="+email+"&invite_code="+invite_code+"&from="+sender_obj.email

                            body = f"Hello,<br/>\
                                    You have been invited to join {den_obj.name} by {sender_obj.name}\
                                    <p>Please <a href='{invitation_link}'>click here</a> to Be part of his den.</p>"
                            ...
                            mail_sent = mailer(subject, from_, to_, body)

                            if mail_sent:
                                return JsonResponse({'success': True, 'message': 'Invitation Mail sent successfully to '+email})
                            else:
                                return JsonResponse({'success': "Exception", 'error': 'UNABLE TO SENT MAIL', 'message': "An exception occurred while sending mail. Please contact system admin."})

            else:
                return JsonResponse({'success': False, 'error': 'Invalid Request',  'message': 'There was some error in sending email. Please try again.'})
        else:
            return JsonResponse({'success': False, 'error': 'Invalid Request', 'message': 'User not Logged in'})
    else:
        return JsonResponse({'success': False, 'error': 'Login Required', 'message': 'User not Logged in'})
                ...
                try:
                    if guest_email == request.user.email:
                        ...
                        if guest_obj == 0:
                            ...
                            try:
                                invite_obj = DenInvitee.objects.filter(invite_code=invite_code, email_to=guest_email)
                                if len(invite_obj) == 1:
                                    invite_obj = invite_obj[0]
                                    hntr_den_map_obj = Hunter_Den_Mapping.objects.filter(den=invite_obj.den, hunter=guest_obj, member_status=True)
                                    if len(hntr_den_map_obj) == 0:
                                        invitee = invite_obj.invitee
                                        if invitee_email == invitee.email:
                                            if invite_obj.status == False:
                                            ...
                                            elif invite_obj.status == True:
                                                # ERROR PAGE AS USER ALREADY PART OF THIS DEN
...
# function use to sent mail
def mailer(subject, from_, to_, body):
    ...
    msg = MIMEMultipart()
    msg['Subject'] = subject
    msg['From'] = smtp_user if from_=='' else from_
    msg['To'] = to_
